...
**Action:**
- Cloned `9router` to `C:\Users\ronsi95openclaw\9router`, installed deps, ran its test suite (1162 pass/46 fail — in line with its own documented baseline of ~938/64), built + ran it as a local gateway on port 20128 with real generated secrets (`JWT_SECRET`, `API_KEY_SECRET`, `MACHINE_ID_SALT`, `INITIAL_PASSWORD`) in its own `.env`, `DATA_DIR` repointed to a local Windows path (repo default `/var/lib/9router` doesn't exist here).
- Added an OpenRouter provider connection inside 9Router (Ronnie's existing `OPENROUTER_API_KEY`, confirmed live end-to-end with a real completion).
- `core/brain.py`: `ask_openrouter()` now routes through 9Router (`NINEROUTER_BASE_URL`/`NINEROUTER_API_KEY` in root `.env`) when set. Tries free OpenRouter models first — `nvidia/nemotron-3-super-120b-a12b:free` → `qwen/qwen3-next-80b-a3b-instruct:free` → `google/gemma-4-31b-it:free` (reordered strongest-first per Ronnie's ask) — and only falls to the paid `openrouter/auto` meta-router as a last resort, capped via `provider.max_price` ($0.5/M prompt, $1.5/M completion) so it can never pick anything expensive. `ask_claude()` itself was left untouched (Anthropic direct) — Ronnie chose OpenRouter-only, not Anthropic-via-9Router.
- Installed the `openai` pip package into `.venv` — it was missing entirely, so `ask_openrouter()` (old or new) could never have actually run before this session.
- **Real bug found via testing, not by design:** killing Ollama mid-request hung `ask_hybrid()` forever (no timeout on the Ollama client) and, because `dashboard/app.py`'s Flask dev server ran without `threaded=True`, that one hung request blocked the *entire* dashboard. Fixed by routing all Ollama calls through a shared `ollama.Client(timeout=...)` (5s status-checks, 60s generation) instead of the untimed module-level `ollama.chat`/`ollama.list`, and adding `threaded=True` to both `app.run()` call sites (`start.py`, `dashboard/app.py`). Also fixed a real pre-existing crash unrelated to this: `core/scheduler.py`'s `AsyncIOScheduler.start()` needs a thread-bound event loop that Python 3.13 no longer auto-creates in non-main threads — added a guard before scheduler construction.
**Verification:**
- Live-tested through the actual running 9Router instance (not a mock): confirmed free-first routing picks the reordered leader (`nemotron-3-super-120b`, cost $0) when available, and correctly falls through to the price-capped `openrouter/auto` when free models are rate-limited (all three genuinely 429'd mid-session from repeated testing — self-inflicted, not a bug).
- Reproduced the exact original hang (killed Ollama mid-request) after the fix and confirmed both parts hold: the dashboard answered other requests immediately while the affected call was still in flight, and that call itself resolved in ~15s instead of hanging forever.
- `usage_stats.json`: caught and fixed a tracking bug I introduced along the way (was about to miscount every OpenRouter call as an Ollama call because `_track_usage()` does an exact string match on `"openrouter"`).
**Result:** ClawBot's OpenRouter fallback path is functional for the first time (missing dependency), now free-tier-first with a hard price ceiling, and the dashboard is materially more robust against a dead Ollama daemon. Restarted ClawBot clean with all fixes live; Ollama and 9Router both confirmed healthy afterward (both were stopped/restarted multiple times during testing).
**Files touched:** `core/brain.py`, `core/scheduler.py`, `start.py`, `dashboard/app.py`, `.env` (added `NINEROUTER_BASE_URL`/`NINEROUTER_API_KEY`), `data/usage_stats.json` (corrected a mis-tracked entry), `.venv` (added `openai` package) — plus the separate `9router/` checkout and its own `.env`, outside this repo.
**Approved by:** Ronnie (interactive, step by step — explicit confirmation given separately for wiring the real OpenRouter key into 9Router's credential store, for the scheduler fix, and for the timeout/threading fix)
**Status:** APPLIED
---
